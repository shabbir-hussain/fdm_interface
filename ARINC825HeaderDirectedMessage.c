/*
 * File: ARINC825HeaderDirectedMessage.c
 *
  *
...
#define S_FUNCTION_NAME ARINC825HeaderDirectedMessage
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
/* Input Port  1 */
#define IN_PORT_1_NAME      SourceAddress
...
/* Input Port  2 */
#define IN_PORT_2_NAME      DestinationAddress
...
/* Input Port  3 */
#define IN_PORT_3_NAME      SourcePortID
...
/* Input Port  4 */
#define IN_PORT_4_NAME      DestinationPortID
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    const uint32_T   *SourceAddress  = (const uint32_T*) ssGetInputPortSignal(S,1);
    const uint32_T   *DestinationAddress  = (const uint32_T*) ssGetInputPortSignal(S,2);
    const uint32_T   *SourcePortID  = (const uint32_T*) ssGetInputPortSignal(S,3);
    const uint32_T   *DestinationPortID  = (const uint32_T*) ssGetInputPortSignal(S,4);
    uint32_T        *Header  = (uint32_T *)ssGetOutputPortRealSignal(S,0);

    ARINC825HeaderDirectedMessage_Outputs_wrapper(LCC, SourceAddress, DestinationAddress, SourcePortID, DestinationPortID, Header);
